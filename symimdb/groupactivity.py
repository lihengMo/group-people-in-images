...
class GroupActivity(IMDB):
    ...
    def __init__(self, image_set, root_path, devkit_path):
        ...
        """
        super(GroupActivity, self).__init__('soa_' + image_set, root_path)
        ...
        self._image_index_file = os.path.join(devkit_path, 'ImageSets', image_set + '.txt')
        self._image_file_tmpl = os.path.join(devkit_path, 'JPEGImages', '{}.jpg')
        self._image_anno_tmpl = os.path.join(devkit_path, 'Annotations', '{}.xml')

        # results
        result_folder = os.path.join(devkit_path, 'results', 'GroupActivity', 'Main')
    ...
    def _load_annotation(self, index):
        ...
        height, width, orig_objs = self._groupactivity_anno(self._image_anno_tmpl.format(index))
    ...
    @staticmethod
    def _groupactivity_anno(filename):
        ...
        for obj in tree.findall('object'):
            ...
            obj_dict['groupnum'] = group.find('groupnum').text
            objects.append(obj_dict)
        return height, width, objects

    def _evaluate_detections(self, detections, use_07_metric=False, **kargs):
    ...
    def _do_python_eval(self, all_boxes, use_07_metric):
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            for roi_rec in self.roidb:
                index = roi_rec['index']
                objects = [obj for obj in roi_rec['objs'] if obj['name'].lower().strip() == cls]
    ...
    def voc_eval(class_anno, npos, image_ids, bbox, confidence, ovthresh=0.5, use_07_metric=False):
        ...
        ap = GroupActivity.voc_ap(rec, prec, use_07_metric)
